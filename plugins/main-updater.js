...
cmd({
    ...
    try {
        await reply("🔍 Checking for HIRU X MD updates...");
        ...
        if (latestCommitHash === currentHash) {
            return reply("✅ Your HIRU X MD bot is already up-to-date!");
        }

        await reply("🚀 Updating HIRU X MD Bot...");

        // Download the latest code
        const zipPath = path.join(__dirname, "latest.zip");
        const { data: zipData } = await axios.get("https://github.com/hirun/HIRU-X-MD/archive/main.zip", { responseType: "arraybuffer" });
        ...
        const sourcePath = path.join(extractPath, "HIRU X MD");
